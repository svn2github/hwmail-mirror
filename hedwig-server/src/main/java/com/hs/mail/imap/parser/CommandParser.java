...
public class CommandParser extends AbstractImapCommandParser {
	...
	public LinkedList<Token> command() {
		if (tag()
			...
			throw new ParseException(tokens, "Syntax error in command");
	}
	...
	private boolean command_auth() {
		return append() || create() || deleteacl() || delete() || examine()
				|| getacl() || getquotaroot() || getquota() || listrights()
				|| list() || lsub() || myrights() || namespace() || rename()
				|| select() || setacl() || setquota() || status()
    ...
    private boolean fetch() {
		if (!kw("FETCH") || !sp() || !sequence_set() || !sp())
			return false;
		if (kw("ALL") || kw("FULL") || kw("FAST") || fetch_att())
			return true;
		if (!lparen())
			return false;
		while (!rparen()) {
    ...
    private boolean fetch_att() {
		if (kw("ENVELOPE") || kw("FLAGS") || kw("INTERNALDATE")
				|| kw("RFC822.HEADER") || kw("RFC822.SIZE")
				|| kw("RFC822.TEXT") || kw("RFC822") || kw("BODYSTRUCTURE")
				|| kw("UID")) {
			return true;
		}
    ...
    private boolean flag() {
		return kw("\\Answered") || kw("\\Flagged") || kw("\\Deleted")
				|| kw("\\Seen") || kw("\\Draft") || flag_keyword()
				|| flag_extension();
	}
    ...
    private boolean header_fld_name() {
		return kw("FROM") || kw("TO") || kw("CC") || kw("BCC") || kw("SUBJECT")
				|| astring();
	}
    ...
    private boolean search_key() {
		if (kw("ALL") || kw("ANSWERED") || kw("BCC") && sp() && astring()
				|| kw("BEFORE") && sp() && date() || kw("BODY") && sp()
				&& astring() || kw("CC") && sp() && astring() || kw("DELETED")
				|| kw("FLAGGED") || kw("FROM") && sp() && astring()
				|| kw("KEYWORD") && sp() && flag_keyword() || kw("NEW")
				|| kw("OLD") || kw("ON") && sp() && date() || kw("RECENT")
				|| kw("SEEN") || kw("SINCE") && sp() && date() || kw("SUBJECT")
				&& sp() && astring() || kw("TEXT") && sp() && astring()
				|| kw("TO") && sp() && astring() || kw("UNANSWERED")
				|| kw("UNDELETED") || kw("UNFLAGGED") || kw("UNKEYWORD")
				&& sp() && flag_keyword() || kw("UNSEEN") || kw("DRAFT")
				|| kw("HEADER") && sp() && header_fld_name() && sp()
				&& astring() || kw("LARGER") && sp() && number() || kw("NOT")
				&& sp() && search_key() || kw("OR") && sp() && search_key()
				&& sp() && search_key() || kw("SENTBEFORE") && sp() && date()
				|| kw("SENTON") && sp() && date() || kw("SENTSINCE") && sp()
				&& date() || kw("SMALLER") && sp() && number() || kw("UID")
				&& sp() && sequence_set() || kw("UNDRAFT") || sequence_set())
			return true;
		if (!lparen())
			return false;
		for (; !rparen(); sp())
    ...
    private boolean section_msgtext() {
		return (kw("HEADER.FIELDS.NOT") || kw("HEADER.FIELDS")) && sp()
				&& header_list() || kw("HEADER") || kw("TEXT");
	}
	...
	private boolean setacl() {
		return kw("SETACL") && sp() && mailbox() && sp() && userid() && sp()
				&& astring();
	}
	...
	private boolean sort_key() {
		return (kw("ARRIVAL") || kw("CC") || kw("DATE") || kw("FROM")
				|| kw("SIZE") || kw("SUBJECT") || kw("TO"));
	}
    
    private boolean status() {
		if (!kw("STATUS") || !sp() || !mailbox() || !sp())
			return false;
		if (!lparen())
			return false;
		while (!rparen()) {
    ...
    private boolean status_att() {
		return kw("MESSAGES") || kw("RECENT") || kw("UIDNEXT")
				|| kw("UIDVALIDITY") || kw("UNSEEN");
	}

	private boolean store() {
		return kw("STORE") && sp() && sequence_set() && sp()
				&& store_att_flags();
	}

    private boolean store_att_flags() {
		if (!kw("+FLAGS.SILENT") && !kw("-FLAGS.SILENT") && !kw("+FLAGS")
				&& !kw("-FLAGS") && !kw("FLAGS.SILENT") && !kw("FLAGS")
				|| !sp())
			return false;
		if (flag_list())
			return true;
